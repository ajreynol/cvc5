    ...
    std::unordered_map<TNode, EvalResult>& results) const
{
  ...
  while (queue.size() != 0)
  {
    ...
    bool isVar = false;
    bool doEval = true;
    // whether the children that are relevant for evaluating currNode have
    // already been determined below
    ...
    {
      // Short circuit: an ITE is determined by its condition and the branch
      // that condition selects, so there is no need to evaluate the other
      // branch. Note the evaluation of ITE below reads only these two.
      itr = results.find(currNode[0]);
      if (itr == results.end())
      {
        // haven't computed the condition yet
      ...
      {
        // computed the condition, only consider the taken branch
        TNode branch = currNode[itr->second.d_bool ? 1 : 2];
      ...
      // Otherwise the condition or the taken branch was invalid, in which case
      // we fall through to the loop below, which queues all children and
      // marks currNode as not evaluatable. This ensures every child has an
      // entry in results before we reconstruct currNode.
    }
