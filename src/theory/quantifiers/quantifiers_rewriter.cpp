    ...
    TConvProofGenerator* pg) const
{
  ...
  if (ret.getKind() == Kind::EQUAL)
  {
    for (size_t i = 0; i < 2; i++)
    {
      ...
      if (k == Kind::ITE && iteLiftMode != options::IteLiftQuantMode::NONE)
      {
      ...
      else if (k == Kind::APPLY_CONSTRUCTOR && i == 0)
      {
        if (ret[1].getKind() == Kind::APPLY_CONSTRUCTOR)
        {
          if (ret[0].getOperator() == ret[1].getOperator())
          {
            ...
            for (size_t j = 0, nchild = ret[0].getNumChildren(); j < nchild;
                 j++)
            {
              conj.push_back(nm->mkNode(Kind::EQUAL, ret[0][j], ret[1][j]));
            }
            ret = nm->mkAnd(conj);
          }
          else
