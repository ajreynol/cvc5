                         ...
                         TConvProofGenerator* tcpg)
{
  ...
  for (;;)
  {
    ...
    if (state == RewriteStackElement::REWRITE_CHILDREN)
    {
      ...
      // Find the next child that actually requires rewriting. A variable
      // cannot be changed by rewriting, so we append it to the builder
      // directly rather than paying for a stack frame and a pre/post-rewrite
      // round trip for it.
      //
      // NOTE: the original branch skipped every child with getNumChildren()==0.
      // That is not sound on current main: nullary operators can rewrite
      // (SequencesRewriter turns re.all into (re.* re.allchar)), and even
      // constants can (DatatypesRewriter::rewriteConstructor normalizes
      // constant constructor applications). Both cases were caught by
      // regress0. Only isVar() is safe, which narrows the optimization.
      Node childNode;
      while (rewriteStackTop.d_nextChild < numChildren)
      {
        Node c = rewriteStackTop.d_node[rewriteStackTop.d_nextChild++];
        if (c.isVar())
        {
          rewriteStackTop.d_builder << c;
          continue;
        }
        childNode = c;
        break;
