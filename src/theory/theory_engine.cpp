...
TheoryEngine::TheoryEngine(Env& env)
    : EnvObj(env),
      ...
      d_lazyProofs(env.isTheoryProofProducing()
                       ? new CDProofSet<LazyCDProof>(
                             env, userContext(), "TheoryEngine::LazyProofs")
                       : nullptr),
    ...
    if (isProofEnabled())
    {
      ...
      if (texplanation.getGenerator() == nullptr)
      {
        ...
        Node tidn = builtin::BuiltinProofRuleChecker::mkTheoryIdNode(
            nodeManager(), tid);
        lp->addTrustedStep(proven, TrustId::THEORY_LEMMA, {}, {tidn});
                         ...
                         TheoryId from)
{
  ...
  if (isProofEnabled())
  {
    ...
    if (tlemma.getGenerator() == nullptr)
    {
      ...
      lp->addTrustedStep(lemma, TrustId::THEORY_LEMMA, {}, {tidn});
  ...
  if (logicInfo().isSharingEnabled())
  {
    ...
    if (isProofEnabled())
    {
      ...
      else
      {
        ...
        lp->addTrustedStep(conf, TrustId::THEORY_LEMMA, {}, {tidn});
      }
      ...
      else
      {
        if (!CDProof::isSame(fullConflict, conflict))
        {
          ...
          lp->addStep(provenOr, ProofRule::IMPLIES_ELIM, {proven}, {});
          lp->addStep(fullConflictNeg,
